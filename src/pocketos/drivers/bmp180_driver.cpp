...
#if POCKETOS_BMP180_ENABLE_LOGGING
...
BMP180Driver::BMP180Driver() : address(0), initialized(false), oversampling(0) {
    memset(&calibration, 0, sizeof(calibration));
}

bool BMP180Driver::init(uint8_t i2cAddress) {
    address = i2cAddress;
    
    uint8_t chipId = 0;
    if (!readRegister(BMP180_REG_CHIP_ID, &chipId) || chipId != BMP180_CHIP_ID) {
        return false;
    }
...
void BMP180Driver::deinit() {
    initialized = false;
}

BMP180Data BMP180Driver::readData() {
    BMP180Data data;
...
CapabilitySchema BMP180Driver::getSchema() const {
    CapabilitySchema schema;
    schema.addSetting("address", ParamType::STRING, true, "0x77", "", "", "");
    schema.addSetting("driver", ParamType::STRING, true, "bmp180", "", "", "");
    schema.addSetting("tier", ParamType::STRING, true, POCKETOS_BMP180_TIER_NAME, "", "", "");
...
String BMP180Driver::getParameter(const String& name) {
    if (name == "address") return "0x" + String(address, HEX);
    if (name == "driver") return "bmp180";
    if (name == "tier") return POCKETOS_BMP180_TIER_NAME;
    return "";
}

bool BMP180Driver::setParameter(const String& name, const String& value) {
    return false;
}

bool BMP180Driver::writeRegister(uint8_t reg, uint8_t value) {
...
bool BMP180Driver::readRegister(uint8_t reg, uint8_t* value) {
    Wire.beginTransmission(address);
    Wire.write(reg);
...
bool BMP180Driver::readRegisters(uint8_t reg, uint8_t* buffer, size_t len) {
    Wire.beginTransmission(address);
    Wire.write(reg);
    if (Wire.endTransmission() != 0) return false;
...
bool BMP180Driver::readCalibrationData() {
    uint8_t buffer[22];
    if (!readRegisters(BMP180_REG_CAL_AC1, buffer, 22)) return false;
...
int32_t BMP180Driver::readRawTemperature() {
    writeRegister(BMP180_REG_CONTROL, BMP180_CMD_TEMP);
    delay(5);
    uint8_t buffer[2];
    readRegisters(BMP180_REG_RESULT, buffer, 2);
    return (buffer[0] << 8) | buffer[1];
}

int32_t BMP180Driver::readRawPressure() {
    writeRegister(BMP180_REG_CONTROL, BMP180_CMD_PRESS + (oversampling << 6));
    delay(2 + (3 << oversampling));
    uint8_t buffer[3];
    readRegisters(BMP180_REG_RESULT, buffer, 3);
    return ((buffer[0] << 16) | (buffer[1] << 8) | buffer[2]) >> (8 - oversampling);
}

#if POCKETOS_BMP180_ENABLE_REGISTER_ACCESS
const RegisterDesc* BMP180Driver::registers(size_t& count) const {
    static const RegisterDesc BMP180_REGISTERS[] = {
    ...
    count = sizeof(BMP180_REGISTERS) / sizeof(RegisterDesc);
    return BMP180_REGISTERS;
}

bool BMP180Driver::regRead(uint16_t reg, uint8_t* buf, size_t len) {
    if (!initialized || reg > 0xFF) return false;
    return readRegister((uint8_t)reg, buf);
}

bool BMP180Driver::regWrite(uint16_t reg, const uint8_t* buf, size_t len) {
    if (!initialized || reg > 0xFF || len != 1) return false;
    return writeRegister((uint8_t)reg, buf[0]);
}

const RegisterDesc* BMP180Driver::findRegisterByName(const String& name) const {
    size_t count;
